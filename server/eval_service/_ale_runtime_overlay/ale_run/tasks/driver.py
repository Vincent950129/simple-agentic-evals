...
from ale_verifier_capture import invoke_with_capture, write_sidecar
...
class TaskDriver:
    ...
    def __init__(
        ...
        self._task_path = task_path.strip().strip("/")
    ...
    async def evaluate(self) -> Dict[str, Any]:
        ...
        for attempt in range(_CUA_SESSION_ATTEMPTS):
            try:
                result, verifier_payload = await invoke_with_capture(
                    evaluate_fn, self._task_path, task_cfg, self._session,
                )
                write_sidecar(verifier_payload)
            except Exception as e:
                last_error = e
                if not _is_transient_cua_error(e):
